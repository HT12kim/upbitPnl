...
def _calculate_rsi(close: pd.Series, window: int = 14) -> pd.Series:
    ...
    # Wilder 방식 RSI를 pandas 연산으로 재현합니다.
...
def _calculate_bollinger(close: pd.Series, window: int = 20, num_std: float = 2.0) -> tuple[pd.Series, pd.Series, pd.Series]:
...
def trading_strategy(
    ...
    # RSI, MACD, 볼린저밴드를 외부 라이브러리 없이 계산합니다.
    df['RSI'] = _calculate_rsi(df['close'], window=14)
    df['MACD'], df['MACD_signal'], df['MACD_histogram'] = _calculate_macd(df['close'])
    df['BB_upper'], df['BB_mid'], df['BB_lower'] = _calculate_bollinger(df['close'])
