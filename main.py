...
def get_account_info():
    ...
    # 현재 대상 자산 기준으로 보유 여부를 확인합니다.
    asset_ticker = ASSET
    is_asset_in_account = False
    asset_balance = '0'
    ...
    if asset_ticker in my_account['currency'].values:
        is_asset_in_account = True
        asset_balance = my_account[my_account['currency'] == asset_ticker]['balance'].values[0]
        asset_avg_buy_price = float(my_account[my_account['currency'] == asset_ticker]['avg_buy_price'].values[0])

    logger.debug(f'is_asset_in_account : {is_asset_in_account}')
    logger.debug(f'asset_balance : {asset_balance}')
    logger.debug(f'asset_avg_buy_price : {asset_avg_buy_price}')
    ...
    return {
        'is_asset': is_asset_in_account,
        'asset_balance': asset_balance,
        'asset_buy_price': asset_avg_buy_price,
...
def get_data():
    # 대상 자산 5분봉 가져오기
    asset_5min_data = get_min_candle_data(MARKET, 5)

    return asset_5min_data
...
def auto_trading():
    try:
        ...
        current_position = 1 if account_info['is_asset'] else 0
        ...
        if current_position == 0:
            ...
            if multiple_of_five:
                ...
                if trade_strategy_result['signal'] == 'buy':
                    ...
                    # 매수
                    buy_result = buy_market(MARKET, account_info['krw_available'])
                    ...
                    if buy_result['uuid'].notnull()[0]:
                        ...
                        logger.info(f'[{MARKET}] {account_info["krw_available"]}원 매수 하였습니다.')

                        send_email(f'[{MARKET}] 시장가 매수', trade_strategy_result['message'])
                    else:
        ...
        # 매도
            ...
            if multiple_of_five:
                trade_strategy_result = trading_strategy(get_data(), current_position, buy_time,
                                                         account_info['asset_buy_price'])

                logger.debug(f'trade_strategy_result : {trade_strategy_result}')

                if trade_strategy_result['signal'] == 'sell':
                    sell_result = sell_market(MARKET, account_info['asset_balance'])
                    if sell_result['uuid'].notnull()[0]:
                        while True:
                            open_order_df = get_open_order(MARKET, 'wait')
                            print(open_order_df)
                        ...
                        logger.info(f'[{MARKET}] {account_info["asset_balance"]} 매도 하였습니다.')
