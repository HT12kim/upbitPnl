...
각 시장별 검증된 winner 파라미터:
  ...
  - ETH W1   : vol=2.5  atr=2.0  vbase=20  lb=10  hold=24  tp=2.5  sl=1.0  tp1=off      kr_night/weekday
...
def has_position(account: dict, cfg: MarketConfig) -> bool:
    return cfg.asset in account and float(account[cfg.asset]["balance"]) > 0
...
def _combined_status_msg(now: datetime, snapshots: dict, krw_balance: float) -> str:
...
def auto_trading() -> None:
    try:
        ...
        if now.hour != _last_status_hour:
            try:
                msg = _combined_status_msg(now, snapshots, account["krw_balance"])
                if send_telegram(msg):
                    _last_status_hour = now.hour
                    _clear_hourly_events()
                else:
